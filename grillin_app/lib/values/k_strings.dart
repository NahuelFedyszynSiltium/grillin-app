class KStrings {
  //COMMONS
  static const String accept = "Aceptar";
  static const String all = "Todos";
  static const String board = "Tablero";
  static const String concept = "Concepto";
  static const String minus = "Menos";
  static const String more = "Más";
  static const String importValue = "Importe";
  static const String tapAgainToExit = "Presiona de nuevo para salir";
  static const String youSpend = "Gastaste un";

  // ------------------------------------------------------------------

  //ADD EXPENSE POPUP/PAGE
  static const String expensesAddExpenseFailure =
      "Ocurrió un error al registrar tu consumo";
  static const String expensesSavesExpenseHint =
      "Agregar un nuevo ahorro lo sumará al balance en lugar de restarlo como ocurré con las demás categorías";
  static const String expensesAddNewConcept = "Agregar nuevo concepto";

  //CATEGORIES
  static const String categoryNameSaves = "Ahorros";
  static const String categoryNamePersonals = "Personales";
  static const String categoryNameDailys = "Fijos";
  static const String categoryNameAchievemnts = "Metas";

  //CHANGE PERCENTS PAGE
  static const String changePercents = "Cambiar porcentajes";
  static String changePercentsErrorFailedToGetPercents =
      "Ocurrió un error al obtener los valores de porcentajes";
  static String changePercentsErrorFailedToUpdatePercents =
      "Ocurrió un error al intentar actualizar tus porcentajes";
  static const String changePercentsSelectedTotalPercent =
      "Porcentaje total seleccionado";

  static String changePercentsSuccessUpdatedPercents =
      "Porcentajes actualizados correctamente";

  //GRAPHS PAGE
  static const String graphs = "Gráficos";
  static const String graphsPageActualCicleComparison =
      "Con respecto al ciclo actual";
  static const String graphsPageCicle = "Por Ciclo";
  static const String graphsPageDifferenceActual =
      "Diferencia con ciclo actual";
  static const String graphsPageDifferencePrevious =
      "Diferencia con ciclo anterior";
  static const String graphsPageErrorEmptyData = "No hay datos para mostrar";
  static const String graphsPageErrorFailedToGetData =
      "Ocurrió un error al obtener los datos";
  static const String graphsPageHistory = "Histórico";
  static const String graphsPagePreviousCicleComparison =
      "Con respecto al ciclo anterior";
  static const String graphsPagePleaseSelectCicle =
      "Por favor, selecciona un ciclo para obtener sus datos";
  static const String graphsPageTotalIncome = "Ingresos del ciclo";
  static const String graphsPageTotalExpenses = "Gastos totales";
  static const String graphsPageTotalExpensesAchievements =
      "Total de gastos en metas";
  static const String graphsPageTotalExpensesDaily = "Total de gastos fijos";
  static const String graphsPageTotalExpensesPersonals =
      "Total de gastos personales";
  static const String graphsPageTotalSaves = "Ahorros totales";

  //HISTORY PAGE
  static const String history = "Historial";
  static String historyErrorFailedToGetExpensesHistory =
      "Ocurrió un error al intentar obtener el historial de transacciónes";
  static String historyErrorFailedEmptyExpensesHistory =
      "No existen transacciónes para mostrar";
  static const String historyTableHeaderCicle = "Ciclo";
  static const String historyTableHeaderConcept = "Concepto";
  static const String historyTableHeaderAmount = "Monto";
  static const String historyTableHeaderDate = "Fecha";

  //HOME PAGE
  static const String homeCardNameSaves = "Ahorros";
  static const String homeCardNamePersonals = "Para gastos personales";
  static const String homeCardNameDailys = "Para gastos fijos";
  static const String homeCardNameAchievemnts = "Para metas";
  static String homeErrorFailedToGetBoardData =
      "Ocurrió un error al obtener los balances actuales";

  //SET INCOME PAGE
  static const String setFixedIncome = "Setear ingreso fijo";
  static const String setFixedIncomeButtonString =
      "Setear const ingreso fijo y empezar nuevo ciclo";
  static const String setFixedIncomePageHint =
      "Setea const un ingreso fijo e inicia un nuevo ciclo para guardar todas las transacciones hasta el momento, y volver a const calcular los porcentajes para un nuevo ciclo";
  static const String setFixedIncomes = "Setear ingresos fijos";
  static const String setFixedIncomeSetNewIncomeError =
      "Ocurrió const un error al intentar iniciar un nuevo ciclo";
  static const String setFixedIncomeSetNewIncomeSuccess =
      "Ciclo iniciado correctamente";

  //TRANSFER SAVINGS PAGE
  static const String transferSavings = "Transferir ahorros";
  static const String transferSavingsTotalSaves = "Ahorros totales";
  static const String transferSavingsTransferTo = "Transferir a";

  // ------------------------------------------------------------------

  //ERRORS
  static const String errorsDefaultErrorMessage = "Ocurrió un error inesperado";
  static const String errorsDefaultEmptyDataErrorMessage =
      "No existen datos para mostrar";
  static const String errorsSetFixedAmountFirst =
      "Setea un ingreso fijo desde el menú para iniciar un ciclo";

  //SUCCESS
  static const String successDefaultSavedDataSuccesfullyMessage =
      "Datos guardados correctamente";
}
